...
async function getUsuariosCadastrados(req, res) {
  try {
...
async function getDetalhesUser(req, res) {
  try {
...
async function getEditarUsuario(req, res) {
  try {
...
async function getProdutosAdm(req, res) {
  try {
    const [rows] = await pool.query(
...
async function toggleStatusProduto(req, res) {
  try {
...
async function reativarAvaliacao(req, res) {
  try {
    const { id } = req.body;
    if (!id) return res.status(400).json({ error: 'ID obrigatório' });
...
async function suspenderAvaliacao(req, res) {
  try {
    const { id } = req.body;
    if (!id) return res.status(400).json({ error: 'ID obrigatório' });
